...
class ScrollableGraphView: UIScrollView, UIScrollViewDelegate, ScrollableGraphViewDrawingDelegate {
    ...
    private func updateLabels(deactivatedPoints: [Int], activatedPoints: [Int]) {
        ...
        for point in activatedPoints {
            ...
            //Position change
            position.x = position.x + self.getXPosition() // Left space
    ...
    fileprivate func getXPosition() -> CGFloat {
    ...
    private func createPath () -> UIBezierPath {
        ...
        for i in activePointsInterval {
            ...
            if let pointLocation = self.graphViewDrawingDelegate?.graphPoint(forIndex: i).location {
                ...
                //Position change
                location.x = location.x + self.getXPosition() // Left space
            }
            let colorIndex = i%5
...
private class ReferenceLineDrawingView : UIView {
    ...
    private func addLine(withTag tag: String, from: CGPoint, to: CGPoint, in path: UIBezierPath) {
        ...
        leftLabel.font = UIFont(name:"HelveticaNeue-Light", size:TEXTLABEL_FONT) ?? UIFont.systemFont(ofSize: TEXTLABEL_FONT)
        rightLabel.font = UIFont(name:"HelveticaNeue-Light", size:TEXTLABEL_FONT) ?? UIFont.systemFont(ofSize: TEXTLABEL_FONT)
        
        leftLabel.textColor = UIColor.colorFromHex(hexString: "#9B9B9B")
        rightLabel.textColor = UIColor.colorFromHex(hexString: "#9B9B9B")
    ...
    private func boundingSize(forText text: String) -> CGSize {
        let TEXTLABEL_FONT = CGFloat(10.0)
        let font = UIFont(name:"HelveticaNeue-Light", size:TEXTLABEL_FONT) ?? UIFont.systemFont(ofSize: TEXTLABEL_FONT)
