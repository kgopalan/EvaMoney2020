...
class FloatingTextField: UITextField {
    ...
    open var titleFadeInDuration:TimeInterval = 0.21
    /// The value of the title disappearing duration
    open var titleFadeOutDuration:TimeInterval = 0.2
    ...
    fileprivate func updateTitleColor() {
        if self.hasErrorMessage {
            ...
            if self.editingOrSelected || self.isHighlighted {
                ...
                self.titleLabel.textColor = self.selectedTitleColor //self.titleColor - change
            }
        }
    }
    ...
    fileprivate func updateTitleLabel(_ animated:Bool = false) {
        ...
        if self.hasErrorMessage {
            titleText = self.titleOrPlaceholder() //self.titleFormatter(errorMessage!) //change
        } else {
    ...
    open func getFont() -> UIFont {
        if let font : UIFont = self.isTitleVisible() ?  UIFont(name: "Helvetica Neue", size: 14) : self.placeholderFont {
            return font
        }
    ...
    fileprivate func updateTitleVisibility(_ animated:Bool = false, completion: (()->())? = nil) {
        let alpha:CGFloat = self.isTitleVisible() ? 1.0 : 1.0
        let frame:CGRect = self.titleLabelRectForBounds(self.bounds, editing: self.isTitleVisible())
        // self.titleLabel.text = (self.isTitleVisible() == true) ? self.titleLabel.text?.uppercased() : self.titleLabel.text?.capitalized
        let updateBlock = { () -> Void in
            self.titleLabel.frame = frame
            self.titleLabel.transform = self.isTitleVisible() == false  ? CGAffineTransform(scaleX: 1.4, y: 1.4) : CGAffineTransform(scaleX: 1.0, y: 1.0)
            self.titleLabel.alpha = alpha == 1.0 ? alpha : 1.0
        }
        if animated {
    ...
    open func titleLabelRectForBounds(_ bounds:CGRect, editing:Bool) -> CGRect {
        let titleHeight = self.titleHeight()
        let tag = self.tag
        let titleWidth = (tag == 6) ? CGFloat(172.0): CGFloat(280.0)//bounds.size.width
        if editing {
            return CGRect(x: 0, y: 0, width: titleWidth, height: titleHeight+5)
        }
        return CGRect(x: 0, y: titleHeight + 4, width: titleWidth, height: titleHeight+5)
    }
